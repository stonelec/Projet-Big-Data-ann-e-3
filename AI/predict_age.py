...
range_n_estimators = range(25, 225, 25)  # le n_estimators est le nombre d'arbres dans la forêt; il va de 10 à 100; On teste toutes les dizaines
...
for idx, n_estimators in enumerate(range_n_estimators):
    ...
    print(f'Nombre d\'arbres: {n_estimators}, Précision: {precision_rf:.4f}, Rappel: {recall_rf:.4f}, F1-Score: {f1_rf:.4f}')

# Calculer les moyennes de la précision, du rappel et du F1-score
mean_precision_score_rf = np.mean(precision_scores_rf)
print(f'Moyenne des scores de précision: {mean_precision_score_rf:.4f}')

mean_recall_score_rf = np.mean(recall_scores_rf)
print(f'Moyenne des scores de rappel: {mean_recall_score_rf:.4f}')

mean_f1_score_rf = np.mean(f1_scores_rf)
print(f'Moyenne des F1-score: {mean_f1_score_rf:.4f}')
...
plt.axhline(y=mean_precision_score_rf, color='r', linestyle='--', label=f'Moyenne = {mean_precision_score_rf}')
plt.xlabel('Nombre d\'arbres')
plt.ylabel('Précision')
plt.title('Précision en fonction du nombre d\'arbres pour Random Forest')
plt.legend(loc='best')
plt.grid(True)
plt.show()
# Rappel
...
plt.axhline(y=mean_recall_score_rf, color='r', linestyle='--', label=f'Moyenne = {mean_recall_score_rf}')
plt.xlabel('Nombre d\'arbres')
plt.ylabel('Rappel')
plt.title('Rappel en fonction du nombre d\'arbres pour Random Forest')
plt.legend(loc='best')
plt.grid(True)
plt.show()
# F1-score
...
plt.axhline(y=mean_f1_score_rf, color='r', linestyle='--', label=f'Moyenne = {mean_f1_score_rf}')
plt.xlabel('Nombre d\'arbres')
plt.ylabel('F1-score')
plt.title('F1-score en fonction du nombre d\'arbres pour Random Forest')
plt.legend(loc='best')
plt.grid(True)
plt.show()
...
for idx, n_estimators in enumerate(range_n_estimators):
    ...
    plt.grid(True)
    plt.show()
    print(f'Valeur AUC pour le modèle Random Forest avec {n_estimators} arbres: {roc_auc_global:.2f}')
    """
    tab_auc_rf.append(roc_auc_global)

# Moyenne des AUC
mean_auc_rf = np.mean(tab_auc_rf)
print(f'Moyenne des AUC pour le modèle Random Forest: {mean_auc_rf:.2f}')
...
plt.figure(figsize=(10, 6))
sns.heatmap(conf_matrix, annot=True, fmt='d', cmap='Blues', xticklabels=['0-10', '10-50', '50-100', '100-200'],
            yticklabels=['0-10', '10-50', '50-100', '100-200'])
plt.xlabel('Prédictions')
plt.ylabel('Vérités terrain')
plt.title('Matrice de confusion pour le meilleur modèle K-Nearest Neighbors')
plt.show()
...
plt.axhline(y=mean_precision_score_knn, color='r', linestyle='--', label=f'Moyenne = {mean_precision_score_knn:.2f}')
plt.xlabel('Nombre de voisins')
plt.ylabel('Précision')
...
plt.axhline(y=mean_recall_score_knn, color='r', linestyle='--', label=f'Moyenne = {mean_recall_score_knn:.2f}')
plt.xlabel('Nombre de voisins')
plt.ylabel('Rappel')
...
plt.axhline(y=mean_f1_score_knn, color='r', linestyle='--', label=f'Moyenne = {mean_f1_score_knn:.2f}')
plt.xlabel('Nombre de voisins')
plt.ylabel('F1-score')
...
for n_neighbors in range_n_neighbors:
    ...
    plt.figure(figsize=(10, 6))
    for i in range(n_classes):
        false_positive_rate_knn, true_positive_rate_knn, thresholds_knn = roc_curve(y_test_bin[:, i], tab_y_prob_knn[n_neighbors - 2][:, i])
        roc_auc = auc(false_positive_rate_knn, true_positive_rate_knn)  # roc_auc = roc _ area under the curve

        tab_fpr_knn.append(false_positive_rate_knn)
        tab_tpr_knn.append(true_positive_rate_knn)
    ...
    plt.show()
    """
    print(f'Valeur AUC pour le modèle K-Nearest Neighbors avec {n_neighbors} voisins: {roc_auc_global:.2f}')
    tab_auc_knn.append(roc_auc_global)

# Moyenne des AUC
mean_auc_knn = np.mean(tab_auc_knn)
print(f'Moyenne des AUC pour le modèle K-Nearest Neighbors: {mean_auc_knn:.2f}')